"""LiteLLM routing and fallback for day07 eComBot."""
...
def _on_success(kwargs, completion_response, start_time, end_time) -> None:
    model = kwargs.get("litellm_params", {}).get("model") or kwargs.get("model", "unknown")
...
def _on_failure(kwargs, completion_response, start_time, end_time) -> None:
    ...
    routing_log.append({"status": "failure", "model": model, "error": type(exc).__name__ if exc else "unknown"})
...
def _make_router(primary: str, backup: str, *, primary_timeout: float = 30.0, num_retries: int = 0) -> Router:
    return Router(
        model_list=[
...
fallback_demo_router = _make_router(
    primary="openrouter/google/non-existent-model",
    backup=BACKUP_MODEL,
    num_retries=1,
)
